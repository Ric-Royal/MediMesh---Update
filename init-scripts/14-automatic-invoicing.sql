...
CREATE TABLE IF NOT EXISTS invoice_line_items (
    ...
    service_id UUID, -- Links to consultation_records, lab_orders, radiology_orders, prescriptions, etc.
...
DECLARE
    ...
    v_total_charges DECIMAL(10,2) := 0;
BEGIN
    -- Only add charges when prescription is dispensed
    IF NEW.status = 'dispensed' AND (OLD.status IS NULL OR OLD.status != 'dispensed') THEN
        ...
        INSERT INTO invoice_line_items (
            ...
            v_invoice_id, 'pharmacy', NEW.id, NEW.prescription_number, 'Pharmacy/Medications',

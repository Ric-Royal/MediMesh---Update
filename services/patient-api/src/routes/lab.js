...
router.put('/orders/:id/status', authorize(['lab-tech', 'doctor', 'admin']), async (req, res) => {
  try {
    const { id } = req.params;
...
router.get('/samples/:barcode', authorize(['lab-tech', 'nurse', 'admin']), async (req, res) => {
  try {
...
router.get('/queue', authorize(['lab-tech', 'nurse', 'admin']), async (req, res) => {
  try {
    const query = `
      SELECT lq.*,
...
// Get lab statistics
router.get('/statistics', authorize(['lab-tech', 'admin', 'manager']), async (req, res) => {
  try {
    const query = `
      SELECT

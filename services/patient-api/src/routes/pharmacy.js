...
router.get('/drugs', authorize(['pharmacist', 'doctor', 'nurse', 'admin']), async (req, res) => {
  try {
...
// Get drug by ID
router.get('/drugs/:id', authorize(['pharmacist', 'doctor', 'nurse', 'admin']), async (req, res) => {
  try {
    const { id } = req.params;
    const query = `
      SELECT d.*, dc.category_name,
...
router.get('/drugs/alerts/reorder', authorize(['pharmacist', 'admin', 'manager']), async (req, res) => {
  try {
    const query = `
      SELECT d.*, dc.category_name
      FROM drugs d
...
router.get('/prescriptions', authorize(['pharmacist', 'doctor', 'nurse', 'admin']), async (req, res) => {
  try {
...
router.get('/prescriptions/:id', authorize(['pharmacist', 'doctor', 'nurse', 'admin']), async (req, res) => {
  try {
    const { id } = req.params;
...
router.get('/statistics', authorize(['pharmacist', 'admin', 'manager']), async (req, res) => {
  try {
    const query = `
      SELECT

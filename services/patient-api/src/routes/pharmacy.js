...
router.get('/drugs', authorize(['doctor', 'nurse', 'pharmacist', 'admin']), async (req, res) => {
  try {
...
// Get drug by ID
router.get('/drugs/:id', authorize(['doctor', 'nurse', 'pharmacist', 'admin']), async (req, res) => {
  try {
    const { id } = req.params;
    const query = `
      SELECT d.*, dc.category_name,
...
// Create drug
router.post('/drugs', authorize(['pharmacist', 'admin']), async (req, res) => {
  try {
    const {
...
// Update drug stock
router.post('/drugs/:id/stock', authorize(['pharmacist', 'admin']), async (req, res) => {
  try {
    const { id } = req.params;
...
router.get('/drugs/alerts/reorder', authorize(['pharmacist', 'admin']), async (req, res) => {
  try {
    const query = `
      SELECT d.*, dc.category_name
      FROM drugs d
...
router.get('/prescriptions', authorize(['doctor', 'nurse', 'pharmacist', 'admin']), async (req, res) => {
  try {
...
router.get('/prescriptions/:id', authorize(['doctor', 'nurse', 'pharmacist', 'admin']), async (req, res) => {
  try {
    const { id } = req.params;
...
router.get('/prescriptions/queue/pending', authorize(['pharmacist', 'nurse', 'admin']), async (req, res) => {
  try {
    const query = `
      SELECT p.*,
             pat.first_name || ' ' || pat.last_name as patient_name,
...
router.get('/statistics', authorize(['pharmacist', 'admin']), async (req, res) => {
  try {
    const query = `
      SELECT
